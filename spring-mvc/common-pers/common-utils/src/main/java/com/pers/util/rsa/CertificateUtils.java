...
public class CertificateUtils {
    ...
    public static String signFileToBase64(String filePath, String keyStorePath, String alias, String password)
            throws Exception {
        return new String(Base64Utils.encode(generateFileSign(filePath, keyStorePath, alias, password)));
    }
    ...
    public static boolean verifyFileSign(String filePath, String sign, String certificatePath) 
        ...
        File file = new File(filePath);
        if (file.exists()) {
            byte[] decodedSign = Base64Utils.decode(sign.getBytes());
            FileInputStream in = new FileInputStream(file);
            FileChannel fileChannel = in.getChannel();
            MappedByteBuffer byteBuffer = fileChannel.map(FileChannel.MapMode.READ_ONLY, 0, file.length());
            signature.update(byteBuffer);
    ...
    public static void main(String[] args) throws Exception{
    	String str = "asdfewfashfuewahfasdhfua";
    	byte[] bt = str.getBytes("utf-8");
    	
		System.out.println(new String(Base64Utils.encode(bt))) ;
		System.out.println(new String(Base64Utils.decode(Base64Utils.encode(bt))));
	}
    ...
    public static boolean verifyBase64SignWithDecrypt(String base64String, String sign, String certificatePath) 
            throws Exception {
        byte[] encryptedData = Base64Utils.decode(base64String.getBytes());
